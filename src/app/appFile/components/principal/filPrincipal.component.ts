...
@Component({
  selector: 'app-filPrincipal',
  templateUrl: './filPrincipal.component.html',
  styleUrls: ['./filPrincipal.component.css']
})
export class FilPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'Archivos SAP';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'ARCHIVOS SAP').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'ARCHIVOS SAP').subscribe(res => {
            if (res.message === 'OK') {
