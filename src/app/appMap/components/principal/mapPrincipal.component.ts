...
@Component({
  selector: 'app-mapPrincipal',
  templateUrl: './mapPrincipal.component.html',
  styleUrls: ['./mapPrincipal.component.css']
})
export class MapPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'MAPA').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'MAPA').subscribe(res => {
            if (res.message === 'OK') {
