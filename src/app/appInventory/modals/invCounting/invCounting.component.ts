...
@Component({
  selector: 'modal-invCounting',
  templateUrl: './invCounting.component.html',
  styleUrls: ['./invCounting.component.css']
})

export class InvCountingComponent implements OnInit {
  uploading: boolean;
  loading: boolean;
  columns: string[];
  header: string[];
  ...
  constructor(private invGeneralInitS: InvGeneralInitService, private alertS: AlertService, private dialog: MatDialog, private invGeneraCountingS: InvGeneraCountingService, private fb: FormBuilder, public dialogRef: MatDialogRef<InvCountingComponent>,
  ...
  save() {
    const pattern =  /^[0-9]+$/;
    if (this.countingType != null) {
      if(pattern.test(this.countingType)){
      ...
      this.alertS.open('¡Por favor ingrese una cantidad!', 'warning')
    }
    } else {
      this.alertS.open('¡Por favor ingrese una cantidad!', 'warning')
    }
