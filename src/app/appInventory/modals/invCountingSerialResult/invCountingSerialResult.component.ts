...
import { invSerialResultComponent } from "../invSerialResult/invSerialResult.component";

@Component({
  selector: 'modal-invCountingSerialResult',
  templateUrl: './invCountingSerialResult.component.html',
  styleUrls: ['./invCountingSerialResult.component.css']
})

export class InvCountingSerialResultComponent implements OnInit {
  uploading: boolean;
  loading: boolean;
  columns: string[];
  header: string[];
  ...
  constructor(private invGeneralInitS: InvGeneralInitService, private alertS: AlertService, private dialog: MatDialog, private invGeneraCountingS: InvGeneraCountingService, private fb: FormBuilder, public dialogRef: MatDialogRef<InvCountingSerialResultComponent>,
  ...
  serial(id: number) {
    this.dialog.open(invSerialResultComponent, {
        width: '100%',
        data: { id: id }
    }).afterClosed().subscribe(res => {
      this.listArray();
