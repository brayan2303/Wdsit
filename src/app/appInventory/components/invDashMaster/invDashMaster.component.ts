...
import { InvMasterSerialDashService } from "../../services/invMasterSerialDash.service";
...
@Component({
    selector: 'app-invDashMaster',
    templateUrl: './invDashMaster.component.html',
    styleUrls: ['./invDashMaster.component.css']
})
export class InvDashMasterComponent implements OnInit {
    data1: DataModel;
    data2: DataModel;
    ...
    constructor(private InvMasterSerialDashS: InvMasterSerialDashService, private alertS: AlertService, private chartS: ChartService, private InvMasterSerialS:InvMasterSerialService ) {
    ...
    ngOnInit(): void {
        ...
        this.InvMasterSerialDashS.invStatus().subscribe(res => {
        ...
        this.interval = setInterval(() => {
            ...
            if (this.seconds === 0) {
                ...
                if (this.minutes === -1) {
                    this.loading = true;
                    this.InvMasterSerialDashS.listPerson().subscribe(res => {
                        if (res.message === 'OK') {
    ...
    details() {
        this.countingList=[];
        this.loading = true;
        this.InvMasterSerialDashS.listStatus().subscribe(res => {
            if (res.message === 'OK') {
                this.loading = false;
    ...
    getMissingCounts() {
        this.loading = true;
        this.InvMasterSerialS.missingCount().subscribe(resP => {
          if (resP.message === 'OK') {
      ...
      getMissingCountFound() {
        this.loading = true;
        this.InvMasterSerialS.missingCountFound().subscribe(resP => {
          if (resP.message === 'OK') {
            this.InvMasterSerialEntity = resP.object;
      ...
      getSpareCount() {
        this.loading = true;
        this.InvMasterSerialS.spareCount().subscribe(resP => {
          if (resP.message === 'OK') {
            this.InvMasterSerialEntity = resP.object;
      ...
      getSpareCountFound() {
        this.loading = true;
        this.InvMasterSerialS.spareCountFound().subscribe(resP => {
          if (resP.message === 'OK') {
            this.InvMasterSerialEntity = resP.object;
