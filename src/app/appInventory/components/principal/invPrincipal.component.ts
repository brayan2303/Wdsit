...
@Component({
  selector: 'app-invPrincipal',
  templateUrl: './invPrincipal.component.html',
  styleUrls: ['./invPrincipal.component.css']
})
export class InvPrincipalComponent implements OnInit {
  ...
  ngOnInit() {
    window.document.title = 'Inventario';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'INVENTARIO').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'INVENTARIO').subscribe(res => {
            if (res.message === 'OK') {
