...
import { InvSerialExpressTigoComponent } from 'src/app/appInventory/modals/inventoryTigoModal/invSerialExpressTigo/invSerialExpressTigo.component';
@Component({
    selector: 'app-invMasterInitTigo',
    templateUrl: './invMasterInitTigo.component.html',
    styleUrls: ['./invMasterInitTigo.component.css']
})

export class InvMasterInitTigoComponent implements OnInit {
    loading: boolean;
    columns: string[];
    @Input() formId: number;
    ...
    constructor(private fb: FormBuilder, private InvMasterSerialsLogS:InvMasterSerialsLogService, private InvMasterInitTigoS: InvMasterInitTigoService, private dialog: MatDialog, private alertS: AlertService) {
        this.loading = false;
        ...
        this.InvMasterSerialTigoEntity = new InvMasterSerialEntity();
    }
    ...
    search() {
        this.InvMasterInitTigoS.list(this.genPersonEntity.id).subscribe(res => {
            if (res.message === 'OK') {
                this.loading = false;
    ...
    save() {
        if (this.formId === 0) {
            ...
            this.InvMasterInitTigoS.create(this.form.value, this.genPersonEntity.id).subscribe(resL => {
                if (resL.message === 'OK') {
                    if (resL.object != 0) {
    ...
    getPallet() {
        this.InvMasterInitTigoS.pallet(this.genPersonEntity.id).subscribe(resP => {
            if (resP.message === 'OK') {
    ...
    serial(id: number, codigoSap: string) {
        this.dialog.open(InvSerialExpressTigoComponent, {
            width: '100%',
    ...
    delete(value: number, pallet:string) {
        this.dialog.open(ConfirmationComponent, {
            ...
            if (res) {
                this.InvMasterInitTigoS.validationPallet(value).subscribe(resF => {
                    if (resF.message === 'OK') {
                        this.InvMasterSerialTigoEntity = resF.object;
                        if (this.InvMasterSerialTigoEntity.validation != 'NO') {
                            this.InvMasterInitTigoS.delete(value).subscribe(res => {
                                if (res.message === 'OK') {
                                    if (res.object != 0) {
