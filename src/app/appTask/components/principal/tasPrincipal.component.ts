...
@Component({
  selector: 'app-tasPrincipal',
  templateUrl: './tasPrincipal.component.html',
  styleUrls: ['./tasPrincipal.component.css']
})
export class TasPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'TAREAS').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'TAREAS').subscribe(res => {
            if (res.message === 'OK') {
