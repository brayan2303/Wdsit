...
@Component({
  selector: 'app-dryIce',
  templateUrl: './dryIce.component.html',
  styleUrls: ['./dryIce.component.css']
})
export class DryIceprincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'HIELO SECO ';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'HIELO SECO').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'HIELO SECO').subscribe(res => {
            if (res.message === 'OK') {
