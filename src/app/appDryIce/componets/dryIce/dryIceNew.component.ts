...
@Component({
  selector: 'app-dryIceNew',
  templateUrl: './dryIceNew.component.html',
  styleUrls: ['./dryIceNew.component.css']
})

export class DryIceNewComponent implements OnInit {
  uploading: boolean;
  loading: boolean;
  columns: string[];
  header: string[];
  ...
  save() {
    this.dryIceS.create(this.genPersonEntity.id, this.form.controls.serial.value).subscribe(res => {
      if (res.message === 'OK') {
        if (res.object != 0) {
          this.alertS.open('Registro creado', 'success');
          this.list();
        } else {
          this.alertS.open('El serial se encuentra en el proceso actualmente', 'warning');
        }
      } else {
  ...
  list(){
    this.dryIceS.list().subscribe(res=>{
      if(res.message === "OK"){
        this.loading= false;
