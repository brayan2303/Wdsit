...
@Component({
    selector: 'app-pqrSupport',
    templateUrl: './pqrSupport.component.html',
    styleUrls: ['./pqrSupport.component.css']
})

export class PqrSupportComponent implements OnInit {
    loading: boolean;
    columns: string[];
    id:number;
    name: string;  
    ...
    search(){
        this.pqrsList= [];
        this.loading=true;
        this.pqrFilesCategoryS.list('Cliente', Number(localStorage.getItem('countryId'))).subscribe(res => {
            if (res.message === 'OK') {
    ...
    tracing(name: string) {
        ...
        this.PqrFilesS.findAll(name,Number(this.countryIdLocal)).subscribe(res => {
            if (res.message === 'OK') {
