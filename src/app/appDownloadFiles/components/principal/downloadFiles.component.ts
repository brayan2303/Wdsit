...
@Component({
  selector: 'app-downloadFilesPrincipal',
  templateUrl: './downloadFiles.component.html',
  styleUrls: ['./downloadFiles.component.css']
})
export class DownloadFilesprincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'DESCARGA ARCHIVOS';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'DESCARGA ARCHIVOS').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'DESCARGA ARCHIVOS').subscribe(res => {
            if (res.message === 'OK') {
