...
@Component({
  selector: 'app-conPrincipal',
  templateUrl: './conPrincipal.component.html',
  styleUrls: ['./conPrincipal.component.css']
})
export class ConPrincipalComponent implements OnInit {
  isOpen: boolean = true;
  ...
  controlPanelList: ConControlPanelEntity[];
  optionList: any[];

  constructor(private router: Router, private render: Renderer2, private genApplicationPersonProfileS: GenApplicationPersonProfileService, private genApplicationS: GenApplicationService, private genSectionS: GenSectionService, private genModuleS: GenModuleService, private conControlPanelS: ConControlPanelService, private genPersonS: GenPersonService, private alertS: AlertService,private dialog:MatDialog) {
    this.isOpen = true;
  ...
  ngOnInit() {
    window.document.title = 'Tableros de Control';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'TABLEROS DE CONTROL').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'TABLEROS DE CONTROL').subscribe(res => {
            if (res.message === 'OK') {
              this.moduleList = res.object;
              this.conControlPanelS.findByPersonId(this.genPersonEntity.id).subscribe(res => {
                if (res.message === 'OK') {
                  this.controlPanelList = res.object;
                  this.optionList.push(this.moduleList);
                  this.optionList.push(this.controlPanelList);
                } else {
                  this.alertS.open(res.message, 'error');
                }
              }, err => {
