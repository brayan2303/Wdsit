...
@Component({
  selector: 'app-traPrincipal',
  templateUrl: './traPrincipal.component.html',
  styleUrls: ['./traPrincipal.component.css']
})
export class TraPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'SEGUIMIENTO').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'SEGUIMIENTO').subscribe(res => {
            if (res.message === 'OK') {
