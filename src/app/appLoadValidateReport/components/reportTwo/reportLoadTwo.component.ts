...
@Component({
  selector: 'app-reportLoadTwo',
  templateUrl: './reportLoadTwo.component.html',
  styleUrls: ['./reportLoadTwo.component.css']
})

export class ReportLoadTwoComponent implements OnInit {
  uploading: boolean;
  loading: boolean;
  columns: string[];
  columnsTwo: string[];
  ...
  constructor(private validateR: ValidateReportService, private alertS: AlertService, private reportValidateLoadCountryS: ReportValidateLoadCountryService) {
    this.loading = false;
    this.columns = ['COUNTRY','PRODUCTION_DATE','MODEL_CODE','MODEL_DESCRIPTION','CPE_CATEGORY','SERIAL 1','MaximoDiag','DIAGNOSTIC'];
    this.header = ['country','processedDate','modelCode','modelDescription','cpeCategory','serialOne','maximoDiag','diagnostic'];
    this.columnsTwo = ['COUNTRY','PRODUCTION_DATE','MODEL_CODE','MODEL_DESCRIPTION','CPE_CATEGORY','SERIAL 1','DIAGNOSTIC'];
    this.headerTwo = ['country','processedDate','modelCode','modelDescription','cpeCategory','serialOne','diagnostic'];
...
list(country:string) {
  this.loading = true;
    this.dataSource = new MatTableDataSource([]);
          this.reportValidateLoadCountryS.listCountryRetry(country).subscribe(res => {
            if (res.message === 'OK') {
              this.loading = false;
