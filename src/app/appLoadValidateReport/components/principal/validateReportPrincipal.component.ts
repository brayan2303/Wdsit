...
@Component({
  selector: 'app-validateReportPrincipal',
  templateUrl: './validateReportPrincipal.component.html',
  styleUrls: ['./validateReportPrincipal.component.css']
})
export class ValidateReportPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'REPORTE MILICOM';
    ...
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'REPORTE MILICOM').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'REPORTE MILICOM').subscribe(res => {
            if (res.message === 'OK') {
