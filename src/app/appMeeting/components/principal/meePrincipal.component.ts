...
@Component({
  selector: 'app-meePrincipal',
  templateUrl: './meePrincipal.component.html',
  styleUrls: ['./meePrincipal.component.css']
})
export class MeePrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'Reuniones';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'REUNIONES').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'REUNIONES').subscribe(res => {
            if (res.message === 'OK') {
