...
import { MeeGroupEditComponent } from "../edit/meeGroupEdit.component";
...
export class MeeGroupListComponent implements OnInit {
    ...
    search(){
        this.meeGroupS.list().subscribe(res => {
            if (res.message === 'OK') {
                this.loading = false;
    ...
    edit(value: number) {
        const dialogRef = this.dialog.open(MeeGroupEditComponent, {
            data: { meGroupformId: value }
        });
        dialogRef.afterClosed().subscribe(resA => {
            this.meeGroupS.list().subscribe(resL => {
                if (resL.message === 'OK') {
                    this.loading = false;
    ...
    delete(value:number) {
        this.dialog.open(ConfirmationComponent, {
            ...
            if (res) {
                this.meeGroupS.delete(value).subscribe(res => {
                    if (res.message === 'OK') {
                        if (res.object != 0) {
