...
@Component({
  selector: 'app-disPrincipal',
  templateUrl: './disPrincipal.component.html',
  styleUrls: ['./disPrincipal.component.css']
})
export class DisPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'Distribucion';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'DISTRIBUCION').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'DISTRIBUCION').subscribe(res => {
            if (res.message === 'OK') {
