...
@Component({
  selector: 'app-masPrincipal',
  templateUrl: './masPrincipal.component.html',
  styleUrls: ['./masPrincipal.component.css']
})
export class MasPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'Correo masivo';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'CORREO MASIVO').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'CORREO MASIVO').subscribe(res => {
            if (res.message === 'OK') {
