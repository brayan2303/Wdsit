...
@Component({
    selector: 'app-actListCompany',
    templateUrl: './actListCompany.component.html',
    styleUrls: ['./actListCompany.component.css']
})

export class ActListCompanyComponent implements OnInit {
    ...
    constructor(private activeFixedAprovS: ActiveFixedInventoryService, private dialog: MatDialog, private alertS: AlertService) {
        this.loading = false;
        this.columns = ['identification', 'name', 'product', 'serial', 'personRes'];
    ...
    listAnswer() {
        this.activeFixedAprovS.listCompany().subscribe(res => {
            if (res.message === 'OK') {
                this.loading = false;
