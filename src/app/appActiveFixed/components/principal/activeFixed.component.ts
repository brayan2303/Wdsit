...
@Component({
  selector: 'app-activeFixedPrincipal',
  templateUrl: './activeFixed.component.html',
  styleUrls: ['./activeFixed.component.css']
})
export class ActiveFixedprincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'ACTIVOS').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'ACTIVOS').subscribe(res => {
            if (res.message === 'OK') {
