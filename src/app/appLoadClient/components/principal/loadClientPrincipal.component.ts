...
@Component({
  selector: 'app-loadClientPrincipal',
  templateUrl: './loadClientPrincipal.component.html',
  styleUrls: ['./loadClientPrincipal.component.css']
})
export class loadClientprincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  constructor(private render: Renderer2, private route: Router, private genPersonS: GenPersonService, private genApplicationS: GenApplicationService, private genApplicationPersonProfileS: GenApplicationPersonProfileService, private genSectionS: GenSectionService, private genModuleS: GenModuleService, private alertS: AlertService) {
    this.isOpen = true;
  ...
  ngOnInit(): void {
    window.document.title = 'CARGA CLIENTES';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'CARGA CLIENTES').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'CARGA CLIENTES').subscribe(res => {
            if (res.message === 'OK') {
