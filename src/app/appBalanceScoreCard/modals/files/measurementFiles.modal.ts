...
export class MeasurementFilesModal implements OnInit {
    ...
    constructor(private bscMeasurementDetailS: BscMeasurementDetailService, private alertS: AlertService,
    ...
    ngOnInit() {
        this.loading = true;
        this.bscMeasurementDetailS.listFile(this.data.measurementId,this.data.measurementDetailId).subscribe(resL => {
            if (resL.message === 'OK') {
    ...
    delete(name: string) {
        this.bscMeasurementDetailS.deleteFile(this.data.measurementId,this.data.measurementDetailId, name).subscribe(res => {
            if (res.message === 'OK') {
                if (res.object != 0) {
    ...
    load(file: FileList) {
        if (file != undefined) {
            ...
            if (this.files.length > 0) {
                this.bscMeasurementDetailS.loadFile(this.data.measurementId,this.data.measurementDetailId, this.files).subscribe(res => {
                    if (res.message === 'OK') {
                        if (res.object != 0) {
                            this.files = [];
                            this.loading=true;
