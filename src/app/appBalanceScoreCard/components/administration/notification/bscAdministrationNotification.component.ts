...
@Component({
    selector: 'app-bscAdministrationNotification',
    templateUrl: './bscAdministrationNotification.component.html',
    styleUrls: ['./bscAdministrationNotification.component.css']
})
export class BscAdministrationNotificationComponent implements OnInit {
    loading: boolean;
    editing: number;
    columns: string[];
    ...
    constructor(private bscNotificationS: BscNotificationService,private dialog: MatDialog, private alertS: AlertService) {
        this.loading = false;
        this.editing = 0;
    ...
    ngOnInit() {
        ...
        this.bscNotificationS.list().subscribe(res => {
            if (res.message === 'OK') {
    ...
    onClick() {
        if (this.editing === 0) {
            this.bscNotificationS.create(this.form.value).subscribe(res => {
                if (res.message === 'OK') {
                    if (res.object != 0) {
            ...
            this.bscNotificationS.update(this.form.value).subscribe(res => {
                if (res.message === 'OK') {
                    if (res.object != 0) {
    ...
    delete(mailId: number) {
        this.dialog.open(ConfirmationComponent,{
            ...
            if (res) {
                this.bscNotificationS.delete(mailId).subscribe(resD => {
                    if (resD.message === 'OK') {
                        if (resD.object != 0) {
