...
@Component({
  selector: 'app-bscWorkPlanManage',
  templateUrl: './bscWorkPlanManage.component.html',
  styleUrls: ['./bscWorkPlanManage.component.css']
})
export class BscWorkPlanManageComponent implements OnInit {
  editing: number;
  loading: boolean;
  columns: string[];
  ...
  constructor(private bscPerspectiveS: BscPerspectiveService, private bscStrategicObjetiveS: BscStrategicObjetiveService, private bscWorkPlanS: BscWorkPlanService, private dialog: MatDialog, private alertS: AlertService) {
    this.editing = 0;
    this.loading = false;
  ...
  save() {
    ...
    if (this.editing === 0) {
      this.bscWorkPlanS.create(this.form.value).subscribe(resC => {
        if (resC.message === 'OK') {
          if (resC.object != 0) {
            this.alertS.open('Plan de Trabajo creado!', 'success');
            this.loading = true;
            this.bscWorkPlanS.list(this.strategicObjetiveId).subscribe(resL => {
              if (resL.message === 'OK') {
                this.loading = false;
      ...
      this.bscWorkPlanS.update(this.form.value).subscribe(resU => {
        if (resU.message === 'OK') {
          if (resU.object != 0) {
  ...
  delete(value: number) {
    this.dialog.open(ConfirmationComponent,{
      ...
      if (res) {
        this.bscWorkPlanS.delete(value).subscribe(res => {
          if (res.message === 'OK') {
            if (res.object != 0) {
              this.alertS.open('Plan de trabajo eliminado!', 'success');
              this.loading = true;
              this.bscWorkPlanS.list(this.strategicObjetiveId).subscribe(res => {
                if (res.message === 'OK') {
                  this.loading = false;
