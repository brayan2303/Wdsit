...
@Component({
  selector: 'app-bscPrincipal',
  templateUrl: './bscPrincipal.component.html',
  styleUrls: ['./bscPrincipal.component.css']
})
export class BscPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'Balance Score Card';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'BALANCE SCORE CARD').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'BALANCE SCORE CARD').subscribe(res => {
            if (res.message === 'OK') {
