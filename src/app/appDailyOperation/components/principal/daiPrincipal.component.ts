...
@Component({
  selector: 'app-daiPrincipal',
  templateUrl: './daiPrincipal.component.html',
  styleUrls: ['./daiPrincipal.component.css']
})
export class DaiPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  constructor(private render: Renderer2, private route: Router, private genApplicationPersonProfileS: GenApplicationPersonProfileService, private genPersonS: GenPersonService, private genApplicationS: GenApplicationService, private genSectionS: GenSectionService, private genModuleS: GenModuleService, private alertS: AlertService,private dialog:MatDialog) {
    this.isOpen = true;
  ...
  ngOnInit(): void {
    window.document.title = 'Daily Operation';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'DAILY OPERATION').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'DAILY OPERATION').subscribe(res => {
            if (res.message === 'OK') {
