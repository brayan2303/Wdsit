...
@Component({
    selector: 'modal-detailsListAll',
    templateUrl: 'detailsListAll.modal.html',
    styleUrls: ['./detailsListAll.modal.css']
})
export class DetailsListAll {

    loading: boolean;
    covFormEntity:CovFormEntity;
    constructor(private covFormS:covFormService , private alertS: AlertService,
        public dialogRef: MatDialogRef<DetailsListAll>,
    ...
    ngOnInit(): void {
        this.loading = true;
        this.covFormS.listAll(this.data.id).subscribe(res => {
            if (res.message === 'OK') {
                this.loading = false;
