...
@Component({
    selector: 'modal-detailsWorking',
    templateUrl: 'detailsWorking.modal.html',
    styleUrls: ['./detailsWorking.modal.css']
})
export class DetailsWorking {

    loading: boolean;
    covFormEntity:CovFormEntity;
    constructor(private covFormS:covFormService , private alertS: AlertService,
        public dialogRef: MatDialogRef<DetailsWorking>,
    ...
    ngOnInit(): void {
        this.loading = true;
        this.covFormS.workingFindByIdArray(this.data.id).subscribe(res => {
            if (res.message === 'OK') {
                this.loading = false;
