...
@Component({
  selector: 'app-wlsPrincipal',
  templateUrl: './wlsPrincipal.component.html',
  styleUrls: ['./wlsPrincipal.component.css']
})
export class WlsPrincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  constructor(private render: Renderer2, private route: Router, private genPersonS: GenPersonService, private genApplicationPersonProfileS: GenApplicationPersonProfileService, private genApplicationS: GenApplicationService, private genSectionS: GenSectionService, private genModuleS: GenModuleService, private alertS: AlertService, private dialog: MatDialog) {
    this.isOpen = true;
  ...
  ngOnInit(): void {
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'WLS').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          if (localStorage.getItem('proyectId') != null && Number(localStorage.getItem('proyectId')) != 0) {
            this.genModuleS.findByProfileId(this.genProfileEntity.id, 'WLS').subscribe(res => {
              if (res.message === 'OK') {
  ...
  selectProyect() {
    this.dialog.open(SelectProyectModal, {
      width: '500px'
    }).afterClosed().subscribe(resA => {
      if (resA != 0) {
        this.route.navigate(['wls']);
        this.genModuleS.findByProfileId(this.genProfileEntity.id, 'WLS').subscribe(res => {
          if (res.message === 'OK') {
            this.moduleList = res.object;
          } else {
