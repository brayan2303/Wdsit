...
@Component({
  selector: 'app-repWolkboxPrincipal',
  templateUrl: './repWolkbox.component.html',
  styleUrls: ['./repWolkbox.component.css']
})
export class RepWolkboxprincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'WOLKVOX').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'WOLKVOX').subscribe(res => {
            if (res.message === 'OK') {
