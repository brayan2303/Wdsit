...
import { ProVariableValueModel } from "../../models/proVariableValue.model";
...
export class DetailVariableModal {
    ...
    constructor(private proMeasurementDetailS: ProMeasurementDetailService, private proMeasurementDetailVariableS: ProMeasurementDetailVariableService, private alertS: AlertService,
    ...
    ngOnInit(): void {
        this.proMeasurementDetailVariableS.list(this.data.measurementDetailId).subscribe(res => {
            if (res.message === 'OK') {
    ...
    save() {
        ...
        this.proMeasurementDetailVariableS.update(array).subscribe(resU => {
            if (resU.message === 'OK') {
                if (resU.object != 0) {
                    this.proMeasurementDetailS.update(this.data.measurementDetailId, 'Result', Number(eval(operation))).subscribe(res => {
                        if (res.message === 'OK') {
                            if (res.object != 0) {
