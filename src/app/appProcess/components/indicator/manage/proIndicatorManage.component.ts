...
@Component({
  selector: 'app-proIndicatorManage',
  templateUrl: './proIndicatorManage.component.html',
  styleUrls: ['./proIndicatorManage.component.css']
})
export class ProIndicatorManageComponent implements OnInit {
  editing: number;
  loading: boolean;
  columns: string[];
  ...
  constructor(private proPerspectiveS: ProPerspectiveService, private proStrategicObjetiveS: ProStrategicObjetiveService, 
    private proIndicatorS: ProIndicatorService, private alertS: AlertService,private dialog:MatDialog) {
  ...
  save() {
    if (this.editing === 0) {
      this.proIndicatorS.create(this.form.value).subscribe(resC => {
        if (resC.message === 'OK') {
          if (resC.object != 0) {
            this.alertS.open('Indicador creado!', 'success');
            this.loading = true;
            this.proIndicatorS.list(this.strategicObjetiveId).subscribe(resL => {
              if (resL.message === 'OK') {
                this.loading = false;
      ...
      this.proIndicatorS.update(this.form.value).subscribe(res => {
        if (res.message === 'OK') {
          if (res.object != 0) {
  ...
  delete(value: number) {
    this.dialog.open(ConfirmationComponent,{
      ...
      if (res) {
        this.proIndicatorS.delete(value).subscribe(res => {
          if (res.message === 'OK') {
            if (res.object != 0) {
              this.alertS.open('Indicador eliminado!', 'success');
              this.loading = true;
              this.proIndicatorS.list(this.strategicObjetiveId).subscribe(res => {
                if (res.message === 'OK') {
                  this.loading = false;
