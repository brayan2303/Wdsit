...
@Component({
  selector: 'app-receptionPrincipal',
  templateUrl: './receptionPrincipal.component.html',
  styleUrls: ['./receptionPrincipal.component.css']
})
export class ReceptionPrincipalprincipalComponent implements OnInit {
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  height: string;
  ...
  ngOnInit(): void {
    window.document.title = 'RECEPCION';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'RECEPCION').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'RECEPCION').subscribe(res => {
            if (res.message === 'OK') {
