...
@Component({
  selector: 'app-schPrincipal',
  templateUrl: './schPrincipal.component.html',
  styleUrls: ['./schPrincipal.component.css']
})
export class SchPrincipalComponent implements OnInit {
  activeMenu: boolean;
  isOpen: boolean;
  isAdmin: boolean;
  open: string = '';
  ...
  constructor(public breakpointObserver: BreakpointObserver, private render: Renderer2, private route: Router, private genApplicationPersonProfileS: GenApplicationPersonProfileService, private genPersonS: GenPersonService, private genApplicationS: GenApplicationService, private genSectionS: GenSectionService, private genModuleS: GenModuleService, private alertS: AlertService,private dialog:MatDialog) {
  ...
  ngOnInit(): void {
    ...
    window.document.title = 'Agendamiento';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'AGENDAMIENTO').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'AGENDAMIENTO').subscribe(res => {
            if (res.message === 'OK') {
