...
@Component({
  selector: 'app-ComPrincipal',
  templateUrl: './ComPrincipal.component.html',
  styleUrls: ['./ComPrincipal.component.css']
})
export class ComCommodityEntryPrincipalComponent implements OnInit {
  isOpen: boolean = true;
  reportName: string;
  open: string = '';
  height: string;
  ...
  ngOnInit() {
    window.document.title = 'ACCESORIOS';
    ...
    this.findImage();
    this.genApplicationPersonProfileS.listProfile(this.genPersonEntity.id, 'ACCESORIOS').subscribe(res => {
      if (res.message === 'OK') {
        ...
        if (this.genProfileEntity != null) {
          ...
          this.genModuleS.findByProfileId(this.genProfileEntity.id, 'ACCESORIOS').subscribe(res => {
            if (res.message === 'OK') {
